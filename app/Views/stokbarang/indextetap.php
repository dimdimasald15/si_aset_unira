...
<div class="card mb-3 shadow datalist-stok">
  ...
  <div class="row m-2 btn-datarestorebarang" style="display:none;">
    <a href="barang-tetap-masuk">&laquo; Kembali ke data <?= strtolower($title); ?></a>
  </div>
</div>
...
<script>
  ...
  function hapus(id, namabrg, namaruang) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "post",
          url: "barang-tetap-masuk/hapus/" + id,
          data: {
            nama_brg: namabrg,
  ...
  $(document).ready(function() {
    ...
    $('#btn-restore').on('click', function() {
      ...
      datarestore = $('#table-restore').DataTable({
        ...
        serverSide: true,
        ajax: {
          url: `barang-tetap-masuk/listdatastokbarang?jenis_kat=${jenis_kat}&isRestore=1`,
          data: function(d) {
            d.barang = $('#selectbarang').val()
            d.kategori = $('#selectkategori').val()
            d.lokasi = $('#selectlokasi').val()
  ...
  function detailstokbarang(kd_brg, ruang_id) {
    ...
    window.location.href = `barang-tetap-masuk/detail-barang/${kdbrg}-${loc_id}`;
  }
  ...
  function restore(id, namabrg, namaruang) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "post",
          url: "barang-tetap-masuk/restore/" + id,
          data: {
            nama_brg: namabrg,
            nama_ruang: namaruang,
            jenis_kat: jenis_kat,
    ...
    if (api.count() === 0) { // jika tidak ada data
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: "post",
            url: "barang-tetap-masuk/restore",
            data: {
              jenis_kat: jenis_kat,
            },
            dataType: 'json',
  ...
  function hapuspermanen(id, namabrg, namaruang) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "post",
          url: "barang-tetap-masuk/hapuspermanen",
          data: {
            id: id,
            nama_brg: namabrg,
            nama_brg: namaruang,
