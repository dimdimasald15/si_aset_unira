...
const util = (() => {
    ...
    const togglePasswordVisibility = (checkboxSelector, passwordSelectors) => {
    ...
    const rmValidationError = (selector, errorSelector) => {
        $(selector).on('input change keyup', function (e) {
            e.preventDefault();
    ...
    const toggleElements = (hideElements, showElements) => {
        hideElements.forEach(selector => $(selector).hide(500));
        showElements.forEach(selector => $(selector).show(500));
    }
    const viewModalPhotos = (id, index) => {
        const datas = {
            url: `${nav}/tampilmodalphotos`,
            type: "POST",
            data: { id, index },
        }
        function callback(response) {
            ...
            const carousel = new bootstrap.Carousel(document.querySelector('#lightboxCarousel'));
            carousel.to(index);
